...
function string(str1, str2) {
  return str1.slice(0, 2).toUpperCase().concat(str2.slice(-3).toUpperCase());
...
function even() {
  evenElements = elements.filter(pari => {
    if (pari % 2 === 0) {
      return pari;
    }
  })
...
function sum() {
  return sum2 = evenElements.reduce((elements, item) => elements + item);
}
...
function increments(number) {
  newArrey = evenElements.map(item => {
    return item + number;
  })
}
...
function tFilm() {
  const titleFilm = movies.map(item => item.Title)
  return titleFilm;
}
...
function sumAge() {
  return movies.reduce((sum, item) => sum + Number(item.Year), 0);
}
